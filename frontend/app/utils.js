...
window.addEventListener("DOMContentLoaded", function () {
    ...
    const scrollNow=()=>{
        ...
        const tryScroll=()=>{
            const anchor=location.hash?location.hash.split("!")[0]:undefined;
            if(!anchor||anchor=="#")return;
            // console.log("Try scroll to ",anchor);
            let contentAnchor = document.querySelector(anchor);            
            if(contentAnchor){
                window.scrollTo(0, 0);
                // console.log("Scroll to ",anchor);
                scrollTo(contentAnchor);
            }else {
                // console.log("Not found. Retry in 100 ms");
                scrollTimeout=setTimeout(tryScroll,100);
            }
        };
        tryScroll();
    };
    ...
    window.addEventListener("hashchange", function () {
        if(location.hash&&location.hash.split("!")[0]!="#")scrollNow();
    });
